...
* [wiki MVC](http://zh.wikipedia.org/wiki/MVC)
...
MVC 的模型 Model、视图 View、控制器 Control 三个相互独立又相互联系，C 则是作为其中的桥梁。
...
* **V** 视图是嵌套包含的，可以用组合 Composite 实现；视图需要引用模型（M-V），一个视图可以引用一个或多个模型，视图会收到模型的通知并自动更新，可以用观察者模式 Observer 实现；视图需要响应用户的交互，使用浏览器事件模型。
...
2.  DOM 事件，即我们熟知的 click、focus 等浏览器事件，用来响应用户操作、实现交互。DOM 事件被绑定在视图 View上，特别的是 Backbone 提倡采用集中配置的方式，例如下面的代码：
    ...
    这个过程称为“DOM事件驱动视图“，在 Backbone 由 View 实现，事件的绑定、触发、销毁则依赖于 jQuery 等第三方库。

3.  模型事件，包括业务模型事件和数据模型事件，用来驱动模型以及模型集合，实现模型和视图之间的观察者模式。
    这个过程称之为”模型驱动“，在 Backbone 中由 Model、Collection、Event 实现。
